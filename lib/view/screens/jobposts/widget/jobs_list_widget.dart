...
class JobsListWidget extends StatelessWidget {
  ...
  const JobsListWidget({Key? key, required this.jobs, required this.hasDivider})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        ...
        Get.toNamed(
          RouteHelper.getStoreRoute(id: jobs.id, page: 'item'),
          arguments:
              JobUpdateScreen(jobs: Jobs(id: jobs.id), fromModule: false),
        );
      },
      child: Column(children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ClipOval(
            child: CustomImage(
              image: '${AppConstants.baseUrl}/${jobs.id ?? ''}',
              height: 60,
              width: 60,
              fit: BoxFit.cover,
            ),
                ...
                Text(
                  ...
                  style: TextStyle(
                    ...
                    color: getStatusColor(
                        jobs.status), // Set the color based on statusId
                  ),
                ),
