...
class AuthController extends GetxController implements GetxService {
  ...
  XFile? _pickedLogo;
  XFile? _pickedCover;
  XFile? _pickedOfferCover;
  XFile? _pickedPostPic1;
  XFile? _pickedPostPic2;
  XFile? _pickedPostPic3;
  XFile? _pickedPostPic4;
  ...
  XFile? get pickedOfferCover => _pickedOfferCover;
  XFile? get pickedPostPic1 => _pickedPostPic1;
  XFile? get pickedPostPic2 => _pickedPostPic2;
  XFile? get pickedPostPic3 => _pickedPostPic3;
  XFile? get pickedPostPic4 => _pickedPostPic4;
  ...
  Future<ResponseModel> login(String? phone, String password) async {
    ...
    await prefs.setString('phone', phone!); // Replace 'key' and 'value' with your data
  ...
  void pickImage(int image, bool isRemove) async {
    if(isRemove) {
      _pickedLogo = null;
      _pickedCover = null;
      _pickedOfferCover = null;
      ...
      if (image == 1) {
        ...
        _pickedOfferCover = await ImagePicker().pickImage(source: ImageSource.gallery);
      }
      update();
    }
  }

  void pickPostImage(int image, bool isRemove) async {
    if(isRemove) {
      _pickedPostPic1 = null;
      _pickedPostPic2 = null;
      _pickedPostPic3 = null;
      _pickedPostPic4 = null;

    }else {
      if (image == 1) {
        _pickedPostPic1 = await ImagePicker().pickImage(source: ImageSource.gallery);
      }
      else if(image == 2){
        _pickedPostPic2 = await ImagePicker().pickImage(source: ImageSource.gallery);
      }
      else if(image == 3){
        _pickedPostPic3 = await ImagePicker().pickImage(source: ImageSource.gallery);
      }

      else{
        _pickedPostPic4 = await ImagePicker().pickImage(source: ImageSource.gallery);
      }
      update();
    }
  }
  ...
  Future<void> getZoneList() async {
    ...
    if (response.statusCode == 200) {
      ...
      setLocation(LatLng(
        ...
        // double.parse('12.918804202266855'),
        // double.parse('77.65186298277348'),
      ));
  ...
  Future<void> registerStore(StoreBody storeBody) async {
    _isLoading = true;
    update();
    Response response = await authRepo.registerStore(storeBody, _pickedLogo, _pickedCover, _pickedOfferCover);
    if (response.statusCode == 200) {
      // Successful registration, navigate back to the RegisteredStoresScreen
      Get.offNamed(RouteHelper.allRegisteredStores);
  ...
  Future<void> createPost(PostBody postBody) async {

    _isLoading = true;
    update();
    Response response = await authRepo.createPost(postBody, _pickedPostPic1, _pickedPostPic2, _pickedPostPic3, _pickedPostPic4);
    if(response.statusCode == 200) {
      Get.offNamed(RouteHelper.allPosts);
      if(ResponsiveHelper.isDesktop(Get.context)){
        Get.offAllNamed(RouteHelper.getInitialRoute());
      }else {
        Get.back();
      }
      showCustomSnackBar('Post Created/Updated Successfully', isError: false);
  ...
  Future<void> createJob(JobBody jobBody) async {

    // _isLoading = true;
    update();
    Response response = await authRepo.createJob(jobBody);
    if(response.statusCode == 200) {
      if(ResponsiveHelper.isDesktop(Get.context)){
        Get.offAllNamed(RouteHelper.getInitialRoute());
      }else {
      ...
      showCustomSnackBar('Job Created/Updated Successfully', isError: false);
...
//  Newly added codes - Deepesh Raj
  RxString selectedZone = ''.obs; // Add this variable
