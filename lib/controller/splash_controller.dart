...
import '../data/model/response/area_model.dart';
...
class SplashController extends GetxController implements GetxService {
  ...
  void selectedCityIndex(int index){
  ...
  Future<void> getCities({Map<String, String>? headers}) async {
    _cityIndex = 0;
    Response response = await splashRepo.getCities(headers: headers);
    if (response.statusCode == 200) {
      _cityList = [];
      response.body.forEach((cityData) => _cityList!.add(CityModel.fromJson(cityData)));
    } else {
      ApiChecker.checkApi(response);
    }
    update();
  ...
  Future<void> getAreas({Map<String, String>? headers}) async {
    _areaIndex = 0;
    Response response = await splashRepo.getAreas(headers: headers,stateId: 1);
    if (response.statusCode == 200) {
      _areaList = [];
      response.body.forEach((areaData) => _areaList!.add(AreaModel.fromJson(areaData)));
    } else {
      ApiChecker.checkApi(response);
    }
    update();
